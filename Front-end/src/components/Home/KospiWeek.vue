<template>
  <div class="w-80 mx-auto my-8 border-4 rounded-xl p-2 border-indigo-200 h-56">
    <div class="flex justify-between items-center mb-4 ml-1">
      <span class="text-xl font-semibold animate-rotate-x animate-twice animate-duration-[1200ms]">
        KOSPI 지수 <span class="text-2xl text-indigo-400 font-semibold">{{ kospi_cur }}</span>
      </span>
      <!-- SVG 아이콘 -->
      <div class="relative group">
        <router-link to="/stock/Kospi">
            <div
              class="border-solid border-2 rounded-lg border-indigo-300 text-black mr-1 animate-rotate-y animate-twice animate-duration-[1200ms] bg-indigo-100">
...
onMounted(async () => {
  ...
  try {
    const response = await axios.get<KospiData[]>(`${baseURL}/market/send_main/kospi/`);
    kospi_cur.value = parseFloat(response.data[0].clpr)
    const data = {
    ...
    console.error('Failed to fetch KOSPI data:', error);
  }
});
