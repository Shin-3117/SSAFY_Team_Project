<template>
  <div class="container mx-auto p-6 flex flex-col items-center">
    ...
    <div class="flex items-center mb-4 animate-fade animate-duration-[1800ms]">
      <input v-model="days" placeholder="현재 날짜로부터 계산할 데이터 수 (1개 = 1일)" class="m-2 p-2 border-2 border-gray-300 rounded-md 
    ...
    <div v-if="!dataLoaded">
      <div class="animate-fade animate-duration-[1800ms]">
        <p class="temp font-semibold text-violet-500 text-xl leading-loose ml-40"><br>SS INVEST에서는 금과 석유에 대한
          시세와<br>주가지수(KOSPI, KOSDAQ, KRX, 테마) 시세에 대한 데이터를<br>2022년부터 현재까지 약 7만개 보유중입니다.<br><br>현재 날짜로부터 역순으로 포함할 데이터의 개수를
          입력하시면<br>해당 기간 동안의 모든 데이터를 분석하여 상승 / 하락에 대한<br>추세 지표를 제공합니다.<br>또한 해당 추세를 바탕으로 미래의 지표를 예측해드립니다.<br><br>결과를 참고하여 투자에 도움이 되기를 바랍니다.</p>
      </div>
    </div>
    ...
    <div v-else class="flex flex-wrap flex-col items-center justify-center">
      ...
      <div v-if="investmentData[selectedCategory]"
        class="p-4 border border-gray-200 rounded-lg shadow temp bg-gradient-to-br from-blue-300 to-red-300 animate-fade animate-duration-[1500ms]">
        ...
        <div class="flex justify-between">
          <div class="w-1/2 pr-2 bg-blue-200">
            ...
            <ul class="overflow-auto temp2">
              <li v-for="item in positiveItems" :key="item.Name"
                class="my-2 animate-fade-up animate-duration-[1500ms] ml-2">
                <span class="font-medium">{{ item.Name }}</span>: <span class="font-bold text-blue-600">{{ item.Indicator
                }}</span>
              </li>
            </ul>
          </div>
          ...
          <div class="w-1/2 px-2 bg-yellow-200">
            <h3 class="text-lg font-semibold mb-2">{{ futureDays }}일 후 예측 지수</h3>
            <ul class="overflow-auto temp2">
              <li v-for="item in predictionItems" :key="item.Name" class="my-2 animate-jump animate-duration-1000">
                {{ item.Name }}: <span class="font-bold text-yellow-600">{{ item['Predicted Price'] }}</span>
              </li>
            </ul>
          </div>
        </div>
...
export default defineComponent({
  setup() {
    ...
    const loadData = () => {
      ...
      axios.get(`${baseURL}/market/recommend_data/${days.value}/${futureDays.value}/`)
        .then(response => {
    ...
    const predictionItems = computed(() => {
      return selectedCategory.value && Array.isArray(investmentData.value[selectedCategory.value])
        ? investmentData.value[selectedCategory.value].filter(item => item.Name.includes('예상 가격'))
          .map(item => ({
            Name: item.Name.replace(' 예상 가격', ''), // " 예상 가격" 텍스트 제거
            'Predicted Price': item['Predicted Price']
          }))
        : [];
    });
